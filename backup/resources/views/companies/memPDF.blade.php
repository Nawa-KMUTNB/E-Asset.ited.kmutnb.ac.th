...
<head>
    ...
    @if (
        $company &&
            $company->num_department &&
            $company->fullname &&
            (strpos($query, $company->fullname) !== false || strpos($query, $company->num_department) !== false))
        <title>ครุภัณฑ์ของ : {{ $company->fullname }}</title>
    @elseif(isset($query))
        <title>ใบรายการค้นหาครุภัณฑ์ของ: {{ $query }}</title>
    @else
        <title>ไม่มีรายการคำค้นที่ท่านต้องการ</title>
    @endif
...
<body>
    <div>
        <a class="btn_back" href="{{ route('companies.member') }}" id="back">ย้อนกลับ</a>
    </div>
